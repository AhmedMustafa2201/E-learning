    ...
    lessonCollection.doc(location.search.split('&')[0].split('=')[1]).get()
        .then(res => {
            ...
            <div class="show-lessons">
                <button onclick="location.assign('./resources/${res.data().attached}')" class="show-lessons-Btn">${courseName}</button>
            </div>
        </div>`
            ...
            commentCollection.add({
                ...
                commentCollection.where("lessonID", "==", location.search.split('&')[0].split('=')[1])
                    ...
                    .then(res => {
                        var tmp = ``
                        res.docs.forEach(data => {
                            tmp += `<div  class="comment-user">
                        <img src="${data.data().user_image}"  alt="">
...
document.getElementsByClassName("course-link")[0].onclick = toggleLessons

// window.onresize = function() {
//     let lessons = document.getElementsByClassName('show-lessons')
//     if (window.innerWidth < 992) {
//         for (let i = 0; i < lessons.length; i++) {
//             if (!(lessons[i].classList.contains("hide-lessons")))
//                 lessons[i].classList.add("hide-lessons")
//         }
//     } else {
//         for (let i = 0; i < lessons.length; i++) {
//             if (lessons[i].classList.contains("hide-lessons"))
//                 lessons[i].classList.remove("hide-lessons")
//         }
//     }
// }