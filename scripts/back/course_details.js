(function(){
    ...
    courseCollection.doc(decodeURI(location.search.split('=')[1])).get()
        .then(res=> {
            ...
            document.getElementsByTagName("img")[1].src=`images/server_content/courses/${res.data().img}`
    ...
    lessonCollection.where("courseID", "==", decodeURI(location.search.split('=')[1])).get()
        .then(res=> {
            var tmp=``
            res.docs.forEach((data, i) => {
                console.log(data.data())
                tmp+=`<div class="lesson-content">
                <button onclick="getLessonData('${data.id}')" class="accordion">${i+1}- ${data.data().name}</button>
              </div>`
            });
            tmp+=`<div class="lesson-content">
            <button onclick="location.assign('./quiz .html?name=المحاضرة الأولى')" class="accordion">الاختبار على الدورة</button>
          </div>`
